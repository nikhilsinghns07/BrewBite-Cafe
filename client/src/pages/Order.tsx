...
export default function Order() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Simulate API call and purchase recording
    setTimeout(() => {
      const newOrderId = Math.random().toString(36).substring(2, 10).toUpperCase();
      setOrderId(newOrderId);
      
      // Save purchased items to localStorage to allow reviews
      const purchasedItemIds = items.map(item => item.id);
      const existingPurchases = JSON.parse(localStorage.getItem('cozy-purchases') || '[]');
      // Fix: Use Array.from to avoid TS downlevelIteration error with Set spread
      const updatedPurchases = Array.from(new Set([...existingPurchases, ...purchasedItemIds]));
            ...
            <div>
              <Card className="border-border shadow-sm">
                <CardHeader>
                  ...
                  <CardDescription>Where should we send your order updates?</CardDescription>
                </CardHeader>
                <CardContent>
            ...
            <div>
              <Card className="border-border shadow-sm bg-muted/20">
                ...
                <CardContent>
                  ...
                  >
                    {isSubmitting ? "Processing..." : `Pay $${(cartTotal * 1.08).toFixed(2)}`}
