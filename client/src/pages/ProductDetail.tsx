...
export default function ProductDetail() {
  ...
  useEffect(() => {
    // Check if user has purchased this item (mock persistence)
    if (product) {
      const purchases = JSON.parse(localStorage.getItem('cozy-purchases') || '[]');
      ...
      <main className="flex-grow container mx-auto px-4 py-24 md:py-32">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-20 mb-20">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="block text-muted-foreground mb-1">Preparation Time</span>
                <span className="font-medium">10-15 mins</span>
              </div>
              <div>
                <span className="block text-muted-foreground mb-1">Calories</span>
        ...
        <section className="max-w-3xl mx-auto">
          <div className="flex items-center justify-between mb-8">
            ...
            {!hasPurchased && (
               <p className="text-sm text-muted-foreground italic">Purchase this item to leave a review</p>
            )}
          </div>
            ...
            <div className="bg-card p-6 rounded-xl border border-border mb-10 animate-in fade-in slide-in-from-top-4">
              ...
              <form onSubmit={handleReviewSubmit} className="space-y-4">
                <div className="grid grid-cols-5 gap-2 max-w-[200px] mb-4">
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button key={star} type="button" className="text-amber-400 hover:scale-110 transition-transform">
                      <Star size={24} fill={star <= 5 ? "currentColor" : "none"} />
                    </button>
                  ))}
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <Textarea id="review" placeholder="Tell us what you liked..." className="min-h-[100px]" />
                </div>
          ...
          <div className="space-y-6">
            {product.reviews.length > 0 ? (
              product.reviews.map((review) => (
                <div key={review.id} className="bg-card p-6 rounded-xl border border-border/50 shadow-sm">
              ...
              <p className="text-center text-muted-foreground py-10 italic">No reviews yet. Be the first to try it!</p>
            )}
          </div>
        </section>
      </main>
