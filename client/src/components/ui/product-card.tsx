...
export function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Card className="group overflow-hidden border-none shadow-sm hover:shadow-md transition-all duration-300 bg-card rounded-xl h-full flex flex-col">
      <Link href={`/product/${product.id}`} className="block overflow-hidden relative aspect-[4/3] cursor-pointer">
        <img 
          src={product.image} 
          alt={product.name}
      ...
      <CardContent className="p-4 flex-grow">
        <div className="flex justify-between items-start mb-2">
          ...
          <div className="font-serif font-bold text-lg">${product.price.toFixed(2)}</div>
        </div>
        
        <Link href={`/product/${product.id}`} className="font-serif font-bold text-lg mb-2 block hover:text-primary transition-colors cursor-pointer line-clamp-1">
          {product.name}
        </Link>
